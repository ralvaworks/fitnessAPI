...
module.exports.registerUser = async (req, res) => {
   ...
   try {
      ...
      if (typeof mobileNo !== "string" || !(mobileNo.length >= 11 && mobileNo.length <= 15)) {
      ...
      const newUser = new User({
         ...
         lastName,
         email,
         // The password from the request body is hashed before being saved.
...
module.exports.loginUser = async (req, res) => {
   ...
   try {
      ...
      if (isPasswordCorrect) {
         return res.status(200).json({ access: auth.createAccessToken(user) });  // ok for successful login
      } else {
